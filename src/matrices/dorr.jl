...
"""
struct Dorr{T<:Number} <: AbstractMatrix{T} 
    n::Integer
    M::AbstractMatrix{T}
    ...
    function Dorr{T}(n::Integer, theta::T) where {T<:Number}
        ...
        return new{T}(n, Tridiagonal{T}(c, d, e))
    end
end
...
# functions
@inline Base.@propagate_inbounds function getindex(A::Dorr{T}, i::Integer, j::Integer) where {T}
